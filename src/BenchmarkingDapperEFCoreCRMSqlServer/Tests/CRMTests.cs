...
public class CRMTests
{
    ...
    #region ADO.NET Stored Proc Tests

    private SqlConnection? _connectionADOStoredProc;
    private Name? _namesDataSetADOStoredProc;
    private PhoneNumbers? _phonesDataSetADOStoredProc;
    private Address? _addressesDataSetADOStoredProc;
    private Company? _companiesDataSetADOStoredProc;
    private int _numeroContatosPorCompanhiaADOStoredProc;

    [IterationSetup(Target = nameof(InputDataWithADOStoredProc))]
    public void SetupADOStoredProc()
    {
        _connectionADOStoredProc = new SqlConnection(Configurations.BaseADOStoredProc);
        _namesDataSetADOStoredProc = new Name("pt_BR");
        _phonesDataSetADOStoredProc = new PhoneNumbers("pt_BR");
        _addressesDataSetADOStoredProc = new Address("pt_BR");
        _companiesDataSetADOStoredProc = new Company("pt_BR");
        _numeroContatosPorCompanhiaADOStoredProc = GetNumeroContatosPorCompanhia();
    }

    [Benchmark]
    public Dapper.Empresa InputDataWithADOStoredProc()
    {
        var empresa = new Dapper.Empresa()
        {
            Nome = _companiesDataSetADOStoredProc!.CompanyName(),
            CNPJ = _companiesDataSetADOStoredProc!.Cnpj(includeFormatSymbols: false),
            Cidade = _addressesDataSetADOStoredProc!.City()
        };

        _connectionADOStoredProc!.Open();
        var transaction = _connectionADOStoredProc.BeginTransaction();

        // Usando stored procedure InserirEmpresa
        using (var commandInsertEmpresa = new SqlCommand("InserirEmpresa", _connectionADOStoredProc, transaction))
        {
            commandInsertEmpresa.CommandType = System.Data.CommandType.StoredProcedure;
            commandInsertEmpresa.Parameters.AddWithValue("@CNPJ", empresa.CNPJ);
            commandInsertEmpresa.Parameters.AddWithValue("@Nome", empresa.Nome);
            commandInsertEmpresa.Parameters.AddWithValue("@Cidade", empresa.Cidade);
            empresa.IdEmpresa = Convert.ToInt32(commandInsertEmpresa.ExecuteScalar());
        }

        empresa.Contatos = [];
        for (int i = 0; i < _numeroContatosPorCompanhiaADOStoredProc; i++)
        {
            var contato = new Dapper.Contato()
            {
                IdEmpresa = empresa.IdEmpresa,
                Nome = _namesDataSetADOStoredProc!.FullName(),
                Telefone = _phonesDataSetADOStoredProc!.PhoneNumber()
            };
            // Usando stored procedure InserirContato
            using (var commandInsertContato = new SqlCommand("InserirContato", _connectionADOStoredProc, transaction))
            {
                commandInsertContato.CommandType = System.Data.CommandType.StoredProcedure;
                commandInsertContato.Parameters.AddWithValue("@Nome", contato.Nome);
                commandInsertContato.Parameters.AddWithValue("@Telefone", contato.Telefone);
                commandInsertContato.Parameters.AddWithValue("@IdEmpresa", contato.IdEmpresa);
                contato.IdContato = Convert.ToInt32(commandInsertContato.ExecuteScalar());
            }
            empresa.Contatos.Add(contato);
        }

        transaction.Commit();
        _connectionADOStoredProc.Close();

        return empresa;
    }

    [IterationCleanup(Target = nameof(InputDataWithADOStoredProc))]
    public void CleanupADOStoredProc()
    {
        _connectionADOStoredProc = null;
    }
